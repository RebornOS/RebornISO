...
class SystemdBoot(object):
    ...
    def install(self):
        ...
        conf['default'] = []
        conf['default'].append("title\tReborn\n")
        ...
        entry_path = os.path.join(entries_dir, "reborn.conf")
        with open(entry_path, 'w') as entry_file:
            for line in conf['default']:
                entry_file.write(line)

        entry_path = os.path.join(entries_dir, "reborn-fallback.conf")
        with open(entry_path, 'w') as entry_file:
            for line in conf['fallback']:
                entry_file.write(line)

        if self.settings.get('feature_lts'):
            entry_path = os.path.join(entries_dir, "reborn-lts.conf")
            with open(entry_path, 'w') as entry_file:
                for line in conf['lts']:
                    entry_file.write(line)

            entry_path = os.path.join(
                entries_dir, "reborn-lts-fallback.conf")
            with open(entry_path, 'w') as entry_file:
