...
class Installation(object):
    ...
    def prepare_pacman_keyring():
        ...
        cmd = ["pacman-key", "--populate", "--gpgdir",
               dest_path, "archlinux", "antergos", "rebornos", "aurarchlinux"]
        call(cmd)
        ...
        for mount_point in self.mount_devices:
            ...
            if ("/home" in mount_point and
                ...
                with open(crypttab_path, 'a') as crypttab_file:
                    line = "cryptRebornHome /dev/disk/by-uuid/{0} {1} luks\n".format(
                        uuid, home_keyfile)
                ...
                # Add line to fstab
                txt = "/dev/mapper/cryptRebornHome {0} {1} defaults 0 0".format(
                    mount_point, myfmt)
                all_lines.append(txt)
                logging.debug("Added %s to fstab", txt)
                continue
    ...
    def configure_system(self):
        ...
        if os.path.exists(nanorc_path):
            ...
            with open(nanorc_path, 'a') as nanorc:
                nanorc.write('\n')
                nanorc.write('# Added by Cnchi (Reborn Installer)\n')
